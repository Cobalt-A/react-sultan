...
function CatalogsProduct({ products }: IProducts) {
  ...
  function addOrder(event: React.MouseEvent) {
    ...
    setTimeout(() => {
      (
        document.querySelector(
          `.product__order-btn[datatype="${id}"]`
        ) as HTMLButtonElement
      ).disabled = false;
    }, 1000);
  }
      ...
      {products.map((product) => (
        <div key={product.id} className="col-xxl-4 col-lg-6 product-col">
          <article className="product">
            ...
            <p className="product__barcode product__description">
              Штрихкод: <span>{product.barcode}</span>
            </p>
            <p className="product__manufacturer product__description">
              Производитель: <span>{product.manufacturer}</span>
            </p>
            <p className="product__brend product__description">
