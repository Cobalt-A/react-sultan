...
function CatalogMain() {
  const { filterByPrice, filterByTags, filterByBrand, sortType, products } =
    useAppSelector((state) => state.productReducer);

  const sortedArray = sortProducts(
    sortType.type,
    Array.from(products) as unknown as IProduct[]
  );
  const filtredArray = brandFilter(
    tagsFilter(priceFilter(priceFilter(sortedArray)))
  );
  ...
  function priceFilter(data: IProduct[]) {
    const filtredArray = data.filter((el) => {
      if (
        Number(el.price) > filterByPrice.max ||
        Number(el.price) < filterByPrice.min
      )
        return false;
      return true;
    });
  ...
  function tagsFilter(data: IProduct[]) {
    ...
    const result = data.filter((el) => {
      return el.tags.find((tag) =>
        filterByTags.find((filterTag) => tag === filterTag.id)
      );
    });
